...
class CompanyController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'contact_person_name' => ['required', 'string', 'min:2', 'max:100', 'regex:/^[a-zA-Z\s\.]+$/'],
            'contact_person_mobile' => ['required', 'string', 'regex:/^[6-9]\d{9}$/'],
            'contact_person_position' => ['required', 'string', 'min:2', 'max:100', 'regex:/^[a-zA-Z\s\.\-]+$/'],
            ...
            'contact_person_name.min' => 'Contact person name must be at least 2 characters',
            'contact_person_name.max' => 'Contact person name must not exceed 100 characters',
            'contact_person_name.regex' => 'Contact person name can only contain letters, spaces, and dots',
            'contact_person_mobile.required' => 'Contact person mobile number is required',
            ...
            'contact_person_position.min' => 'Contact person position must be at least 2 characters',
            'contact_person_position.max' => 'Contact person position must not exceed 100 characters',
            'contact_person_position.regex' => 'Contact person position can only contain letters, spaces, dots, and hyphens',
    ...
    public function show(CompanyModel $company): View|RedirectResponse
    {
        ...
        if ($invoices->count() > 0) {
            ...
            $receipts = \App\Models\Receipt::where(function($query) use ($invoiceIds, $company) {
                // Match by invoice IDs in JSON array
            ...
            // If no invoices, try to get receipts by company name
