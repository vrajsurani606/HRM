<?php $__env->startSection('page_title','Check IN / OUT'); ?>
  ...
  <div style="max-width: 500px; margin: 0 auto; background: white; border-radius: 25px; box-shadow: 0 8px 30px rgba(0,0,0,0.12); overflow: hidden;">
    ...
    <div style="background: linear-gradient(135deg, #89CFF0 0%, #67B7D1 100%); padding: 30px 20px 70px; text-align: center; position: relative;">
      ...
      <div style="position: absolute; left: 50%; bottom: -50px; transform: translateX(-50%); width: 100px; height: 100px; border-radius: 22px; overflow: hidden; border: 5px solid white; box-shadow: 0 8px 25px rgba(0,0,0,0.15); z-index: 10; background: white;">
        <?php if($photo): ?>
          <img src="<?php echo e($photo); ?>" alt="avatar" style="width: 100%; height: 100%; object-fit: cover;"/>
        <?php else: ?>
          <span style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; font-weight: 800; font-size: 40px; color: white; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);"><?php echo e($initial); ?></span>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div style="padding: 70px 30px 30px; text-align: center; background: #f8f9fa;">
      <h2 style="font-size: 26px; font-weight: 700; color: #2c3e50; margin: 0 0 5px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><?php echo e($employee->name ?? (auth()->user()->name ?? 'User')); ?></h2>
      <p style="color: #95a5a6; font-size: 15px; margin: 0 0 35px 0; font-weight: 500;">Welcome !</p>

      <!-- Form - visibility controlled by JavaScript -->
      <form id="punchForm" method="POST" action="<?php echo e(route('attendance.check-in')); ?>" style="display: inline-block;">
        <?php echo csrf_field(); ?>
      ...
      <!-- Timing Display -->
      <div id="checkTiming" style="<?php echo e($showTimingImmediately ? '' : 'display:none;'); ?> margin-top: 20px;">
        <div style="display: inline-block; padding: 18px 28px; border-radius: 15px; background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); color: #1976d2; font-weight: 700; font-size: 16px; box-shadow: 0 5px 15px rgba(25, 118, 210, 0.2);">
          <?php if($attendance && $attendance->check_in): ?>
            ✓ Checked in at <?php echo e(\Carbon\Carbon::parse($attendance->check_in)->format('h:i A')); ?>

          <?php endif; ?>
        </div>
        
        <?php if(!$hasCheckOut && $attendance && $attendance->check_in): ?>
        ...
        <?php if($hasCheckOut): ?>
          <div style="margin-top: 12px; color: #e74c3c; font-weight: 700; font-size: 15px;">✓ Checked out at <?php echo e(\Carbon\Carbon::parse($attendance->check_out)->format('h:i A')); ?></div>
          <?php if($attendance->total_working_hours): ?>
            <div style="margin-top: 10px; padding: 12px 20px; border-radius: 10px; background: linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%); color: #2e7d32; font-weight: 700; font-size: 15px;">
              Total Working Time: <?php echo e($attendance->total_working_hours); ?>
        ...
        <div style="margin-top: 25px;">
          <button id="okBtn" type="button" data-dashboard-url="<?php echo e(url('/dashboard')); ?>" style="cursor: pointer; padding: 14px 38px; border-radius: 50px; border: 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-weight: 700; font-size: 15px; box-shadow: 0 6px 18px rgba(102, 126, 234, 0.4); transition: all 0.3s ease;">OK</button>
        </div>
      </div>
      ...
      <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0; color: #7f8c8d; font-size: 13px;">
        <small>
          <?php echo e(\Carbon\Carbon::parse($today)->format('l, d M Y')); ?>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  var hasCheckIn = <?php echo e($hasCheckIn ? 'true' : 'false'); ?>;
  var hasCheckOut = <?php echo e($hasCheckOut ? 'true' : 'false'); ?>;
  
  // Check server status to determine if we can show the form
  function checkServerStatus() {
    fetch('<?php echo e(route("attendance.current-status")); ?>', {
      method: 'GET',
      headers: {
    ...
    .then(data => {
      ...
      if (data.has_checked_in && !data.has_checked_out) {
        ...
        form.action = '<?php echo e(route("attendance.check-out")); ?>';
        startTimer();
        ...
        form.action = '<?php echo e(route("attendance.check-in")); ?>';
  ...
  function startTimer() {
    ...
    <?php if($attendance && $attendance->check_in && !$hasCheckOut): ?>
      var checkInTime = new Date('<?php echo e(\Carbon\Carbon::parse($attendance->check_in)->toIso8601String()); ?>');
...
<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/attendance/check.blade.php ENDPATH**/ ?>