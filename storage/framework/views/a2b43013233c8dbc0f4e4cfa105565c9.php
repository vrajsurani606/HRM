...
<div class="hrp-card">
  <div class="hrp-card-body">
    <form id="filterForm" method="GET" action="<?php echo e(route('employees.letters.index', $employee)); ?>" class="jv-filter">
      <input type="text" name="search" class="filter-pill" placeholder="Search by title, reference..." value="<?php echo e(request('search')); ?>">
      <select name="type" class="filter-pill">
        <option value="" <?php echo e(!request('type') ? 'selected' : ''); ?>>All Types</option>
        <option value="appointment" <?php echo e(request('type') == 'appointment' ? 'selected' : ''); ?>>Appointment</option>
        <option value="offer" <?php echo e(request('type') == 'offer' ? 'selected' : ''); ?>>Offer</option>
        <option value="joining" <?php echo e(request('type') == 'joining' ? 'selected' : ''); ?>>Joining</option>
        <option value="experience" <?php echo e(request('type') == 'experience' ? 'selected' : ''); ?>>Experience</option>
        <option value="relieving" <?php echo e(request('type') == 'relieving' ? 'selected' : ''); ?>>Relieving</option>
        <option value="confirmation" <?php echo e(request('type') == 'confirmation' ? 'selected' : ''); ?>>Confirmation</option>
        <option value="increment" <?php echo e(request('type') == 'increment' ? 'selected' : ''); ?>>Increment</option>
        <option value="warning" <?php echo e(request('type') == 'warning' ? 'selected' : ''); ?>>Warning</option>
        <option value="termination" <?php echo e(request('type') == 'termination' ? 'selected' : ''); ?>>Termination</option>
      ...
      <div class="filter-right">
        ...
        <a href="<?php echo e(route('employees.letters.index', $employee)); ?>" class="pill-btn pill-secondary">Reset</a>
        <a href="<?php echo e(route('employees.letters.create', $employee)); ?>" class="pill-btn pill-success" target="_blank">+ Add</a>
      </div>
    </form>
  </div>
  ...
  <div class="letters-grid-view">
    <?php $__empty_1 = true; $__currentLoopData = $letters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $letter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      ...
      <div class="letter-grid-card" onclick="window.open('<?php echo e(route('employees.letters.print', ['employee' => $employee, 'letter' => $letter])); ?>', '_blank')" title="View letter">
        <div class="letter-grid-header">
          <div class="letter-grid-id">
            <div class="letter-avatar"><?php echo e($initial); ?></div>
            <div class="letter-name-type">
              <h3 class="letter-grid-title"><?php echo e($letter->title); ?></h3>
              <div class="letter-type"><?php echo e($letter->reference_number); ?></div>
            </div>
          </div>
          <div class="letter-chips">
            <span class="chip chip-<?php echo e($letter->type ?? 'other'); ?>"><?php echo e(ucfirst(str_replace('_', ' ', $letter->type ?? 'other'))); ?></span>
          </div>
        </div>
        <p class="letter-grid-sub">Issue Date: <?php echo e($letter->issue_date->format('d M Y')); ?> • Generated: <?php echo e($letter->created_at->format('d M Y')); ?></p>
        <div class="letter-grid-meta">
          <div class="letter-grid-left">
            <div class="meta-row"><span class="meta-label">Employee</span><span class="meta-value"><?php echo e($employee->name ?? 'N/A'); ?></span></div>
            <?php if($letter->notes): ?>
            <div class="meta-row"><span class="meta-label">Notes</span><span class="meta-value"><?php echo e(Str::limit($letter->notes, 80)); ?></span></div>
            <?php endif; ?>
          </div>
          <div class="letter-grid-actions" onclick="event.stopPropagation()">
            <a href="<?php echo e(route('employees.letters.print', ['employee' => $employee, 'letter' => $letter])); ?>" target="_blank" class="letter-grid-action-btn btn-print" title="Print" aria-label="Print">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
            </a>
            <a href="<?php echo e(route('employees.letters.edit', ['employee' => $employee, 'letter' => $letter])); ?>" target="_blank" class="letter-grid-action-btn btn-edit" title="Edit" aria-label="Edit">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
            </a>
            <form method="POST" action="<?php echo e(route('employees.letters.destroy', ['employee' => $employee, 'letter' => $letter])); ?>" class="delete-form" style="display:inline">
  ...
  <div class="letters-list-view active">
    <div class="JV-datatble JV-datatble--zoom striped-surface striped-surface--full table-wrap pad-none">
      <table>
        ...
        <tbody>
          <?php $__empty_1 = true; $__currentLoopData = $letters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $letter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <tr>
            <td>
              <div class="action-icons">
                <a href="<?php echo e(route('employees.letters.print', ['employee' => $employee, 'letter' => $letter])); ?>" title="Print" target="_blank" aria-label="Print">
                  <img src="<?php echo e(asset('action_icon/print.svg')); ?>" alt="Print" class="action-icon">
                </a>
                <a href="<?php echo e(route('employees.letters.edit', ['employee' => $employee, 'letter' => $letter])); ?>" title="Edit" target="_blank" aria-label="Edit">
            ...
            <td><?php echo e($letter->title); ?></td>
            <td class="capitalize"><?php echo e(str_replace('_', ' ', $letter->type ?? 'other')); ?></td>
            <td><?php echo e($letter->issue_date->format('d M Y')); ?></td>
            <td><?php echo e($letter->created_at->format('d M Y')); ?></td>
            <td><?php echo e($employee->name ?? 'N/A'); ?></td>
            <td><?php echo e($letter->notes ? Str::limit($letter->notes, 50) : '—'); ?></td>
          </tr>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
          <tr>
...
<a href="<?php echo e(route('employees.index')); ?>" style="font-weight:800;color:#0f0f0f;text-decoration:none">HRM</a>
<span class="hrp-bc-sep">›</span>
<a href="<?php echo e(route('employees.index')); ?>" style="color:#6b7280;text-decoration:none">Employees</a>
<span class="hrp-bc-sep">›</span>
<span class="hrp-bc-current">Employee Letters</span>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('footer_pagination'); ?>
<?php if(isset($letters) && method_exists($letters,'links')): ?>
...
<?php echo e($letters->appends(request()->except('page'))->onEachSide(1)->links('vendor.pagination.jv')); ?>
...
<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/hr/employees/letters/index.blade.php ENDPATH**/ ?>