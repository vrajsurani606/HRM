...
<div class="hrp-content">
  <!-- Filters -->
  <form method="GET" action="<?php echo e(route('leave-approval.index')); ?>" class="jv-filter">
    <input type="text" name="start_date" class="filter-pill date-picker" placeholder="From : dd/mm/yyyy" value="<?php echo e(request('start_date')); ?>" autocomplete="off">
    <input type="text" name="end_date" class="filter-pill date-picker" placeholder="To : dd/mm/yyyy" value="<?php echo e(request('end_date')); ?>" autocomplete="off">
    ...
    <a href="<?php echo e(route('leave-approval.index')); ?>" class="filter-search" aria-label="Refresh">
    ...
    <div class="filter-right">
      <?php if(auth()->user()->hasRole('super-admin') || auth()->user()->can('Attendance Management.create leave approval')): ?>
      ...
      <tbody>
        ...
        <tr>
          <td style="vertical-align: middle; padding: 14px;">
            <div>
              <?php if($leave->status == 'pending'): ?>
                ...
                <?php if(auth()->user()->hasRole('super-admin') || auth()->user()->can('Attendance Management.approve leave')): ?>
                  <img src="<?php echo e(asset('action_icon/approve.svg')); ?>" alt="Approve" style="cursor: pointer; width: 20px; height: 20px;" onclick="approveLeave(<?php echo e($leave->id); ?>)">
                <?php endif; ?>
                <?php if(auth()->user()->hasRole('super-admin') || auth()->user()->can('Attendance Management.reject leave')): ?>
                  <img src="<?php echo e(asset('action_icon/reject.svg')); ?>" alt="Reject" style="cursor: pointer; width: 20px; height: 20px;" onclick="rejectLeave(<?php echo e($leave->id); ?>)">
                <?php endif; ?>
              <?php else: ?>
                <!-- Edit/Delete buttons for approved/rejected leaves -->
                <?php if(auth()->user()->hasRole('super-admin') || auth()->user()->can('Attendance Management.edit leave approval')): ?>
                  <img src="<?php echo e(asset('action_icon/edit.svg')); ?>" alt="Edit" style="cursor: pointer; width: 18px; height: 18px;" onclick="editLeave(<?php echo e($leave->id); ?>)">
                <?php endif; ?>
                <?php if(auth()->user()->hasRole('super-admin') || auth()->user()->can('Attendance Management.delete leave approval')): ?>
                  <img src="<?php echo e(asset('action_icon/delete.svg')); ?>" alt="Delete" style="cursor: pointer; width: 18px; height: 18px;" onclick="deleteLeave(<?php echo e($leave->id); ?>)">
                <?php endif; ?>
              <?php endif; ?>
            </div>
          </td>
          <td style="vertical-align: middle; padding: 14px 16px;"><?php echo e($leave->employee->code ?? 'N/A'); ?></td>
          <td style="vertical-align: middle; padding: 14px 16px;"><?php echo e($leave->employee->name ?? 'N/A'); ?></td>
          <td style="vertical-align: middle; padding: 14px 16px;">
            <?php echo e(\Carbon\Carbon::parse($leave->start_date)->format('d M, Y')); ?> to <?php echo e(\Carbon\Carbon::parse($leave->end_date)->format('d M, Y')); ?>

          </td>
          <td style="vertical-align: middle; padding: 14px; text-align: center;"><?php echo e($leave->total_days); ?> Day<?php echo e($leave->total_days > 1 ? 's' : ''); ?></td>
          <td style="vertical-align: middle; padding: 14px;">
            <span class="leave-type leave-type--<?php echo e(strtolower($leave->leave_type)); ?>">
              <?php echo e(ucfirst(str_replace('_', ' ', $leave->leave_type))); ?>

              <?php if($leave->is_paid ?? true): ?>
                <?php
                  $yearlyUsed = \App\Models\Leave::where('employee_id', $leave->employee_id)
                    ...
                    ->sum('total_days');
                ?>
                <span style="font-size: 10px; opacity: 0.8;">(Paid <?php echo e($yearlyUsed); ?>/12)</span>
              <?php else: ?>
                <span style="font-size: 10px; opacity: 0.8;">(Unpaid)</span>
              <?php endif; ?>
            </span>
          </td>
          <td style="vertical-align: middle; padding: 14px 16px;"><?php echo e($leave->reason ?? '-'); ?></td>
          <td style="vertical-align: middle; text-align: center; padding: 14px;">
            ...
            ?>
            <span style="color: <?php echo e($statusColor); ?>; font-weight: 600; font-size: 14px;"><?php echo e(ucfirst($leave->status)); ?></span>
          </td>
          <td style="vertical-align: middle; padding: 14px 16px;"><?php echo e($leave->created_at->format('d M, Y, h:i A')); ?></td>
        </tr>
  ...
  <div style="background: white; border-radius: 15px; padding: 30px; max-width: 500px; width: 90%; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
    ...
    <form id="addLeaveForm" onsubmit="submitLeave(event)">
      <div style="margin-bottom: 15px;">
        ...
        <select name="employee_id" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;" <?php echo e($employees->count() == 1 ? 'readonly' : ''); ?>>
          <option value="">Select Employee</option>
          <?php $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $emp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($emp->id); ?>" <?php echo e($employees->count() == 1 ? 'selected' : ''); ?>><?php echo e($emp->name); ?></option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
      </div>
...
function submitLeave(event) {
  ...
  fetch('<?php echo e(route("leave-approval.store")); ?>', {
    method: 'POST',
    headers: {
...
function updateLeaveStatus(id, status) {
  ...
  fetch(`<?php echo e(url('leave-approval')); ?>/${id}`, {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
      'Accept': 'application/json',
...
<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/hr/attendance/leave-approval.blade.php ENDPATH**/ ?>