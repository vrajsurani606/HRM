<?php $__env->startSection('page_title', 'Project Overview'); ?>
...
<link rel="stylesheet" href="<?php echo e(asset('css/project-overview-clean.css')); ?>">
  ...
  <div class="project-header-section">
    <div class="project-header-top">
      ...
      <div class="project-actions-btns">
        <button class="btn-project-action btn-back" onclick="window.location.href='<?php echo e(route('projects.index')); ?>'">Back</button>
        <button class="btn-project-action btn-edit-project" onclick="window.location.href='<?php echo e(route('projects.index')); ?>'">Edit Project</button>
      </div>
    </div>
    <!-- Main Progress Bar -->
  ...
  <div class="tabs-container-section">
    ...
    <div class="tab-content-area active" id="tab-overview">
      ...
      <div class="quick-actions-bar">
        ...
        <button class="action-btn secondary" onclick="window.location.href='<?php echo e(route('projects.index')); ?>'">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function loadProjectData() {
    try {
        ...
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}`, {
            headers: { 
...
async function loadTasks() {
    try {
        ...
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/tasks`, {
...
async function loadTeamMembers() {
    try {
        ...
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/members`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
        });
        const data = await response.json();
...
async function loadComments() {
    try {
        ...
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/comments`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
        });
        const data = await response.json();
...
async function toggleTask(taskId, isCompleted) {
    try {
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/tasks/${taskId}`, {
            method: 'PUT',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json',
...
async function addNewMember() {
    ...
    try {
        ...
        const response = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/available-users`, {
            headers: { 
                'X-Requested-With': 'XMLHttpRequest', 
                'Accept': 'application/json' 
            }
        ...
        const employeeCards = data.users.map(user => {
            ...
            let avatarContent = '';
            if (user.photo_path) {
                avatarContent = `<img src="<?php echo e(asset('storage')); ?>/${user.photo_path}" alt="${escapeHtml(user.name)}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
            } else {
        ...
        if (result.isConfirmed && result.value) {
            ...
            for (const member of result.value.members) {
                try {
                    const addResponse = await fetch(`<?php echo e(url('/projects')); ?>/${projectId}/members`, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'Accept': 'application/json',
...
function uploadFiles(files) {
    ...
    fetch(`<?php echo e(url('/projects')); ?>/${projectId}/files`, {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('API URL:', `<?php echo e(url('/projects')); ?>/${projectId}`);
...
<script src="<?php echo e(asset('js/project-materials.js')); ?>"></script>

<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/projects/overview.blade.php ENDPATH**/ ?>