<header class="hrp-header">
  <div class="hrp-header-left">
    ...
    <h1 class="hrp-page-title"><?php echo $__env->yieldContent('page_title','Dashboard'); ?></h1>
  </div>
  <div class="hrp-header-right" style="display: flex; align-items: center;">
    ...
    <?php if(auth()->user()->can('Attendance Management.check in') || auth()->user()->can('Attendance Management.check out') || auth()->user()->can('Attendance Management.view own attendance')): ?>
    <a href="<?php echo e(route('attendance.check')); ?>" class="hrp-thumb" title="IN/OUT" id="attendanceBtn" style="text-decoration:none; cursor: pointer; position: relative; display: flex; flex-direction: column; align-items: center; justify-content:center; gap: 6px; padding: 10px 12px; border-radius: 12px; background: #E8E3DF; margin-right: 10px; transition: all 0.3s ease; min-width: 70px;">
      <div class="ico" aria-hidden="true" style="display:flex; align-items:center; justify-content:center; width:40px; height:40px;">
        <img src="<?php echo e(asset('action_icon/Vector.svg')); ?>" alt="IN/OUT" width="40" height="40" style="display:block; object-fit: contain;" />
      </div>
    ...
    <div class="dropdown">
      <button class="hrp-user-btn" data-toggle="dropdown" aria-expanded="false">
        ...
        ?>
        <img class="hrp-avatar" src="<?php echo e($photoUrl); ?>" alt="user"/>
        <div class="hrp-user-meta">
          <div class="hrp-user-email"><?php echo e($user->email ?? 'user@example.com'); ?></div>
          <div class="hrp-user-name"><?php echo e($user->name ?? 'User'); ?></div>
        </div>
...
<?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/partials/header.blade.php ENDPATH**/ ?>