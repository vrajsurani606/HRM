<?php $__env->startSection('page_title', 'Event Gallery'); ?>

<?php $__env->startPush('styles'); ?>
<link rel="stylesheet" href="<?php echo e(asset('new_theme/css/events.css')); ?>">
<style>
  /* Full-page gallery */
...
<div class="gallery-page">
  ...
  <div class="jv-filter gallery-toolbar">
    <div class="filter-group" role="tablist">
      <a href="<?php echo e(route('events.index')); ?>" class="pill-btn back-btn" title="Back to Events">
    ...
    <div class="filter-right">
      ...
      <div id="bulkActionsWrap" class="bulk-actions" style="display:none;">
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.download event image')): ?>
        ...
        <?php endif; ?>
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.delete event image')): ?>
        <button id="bulkDelete" class="bulk-btn delete" disabled title="Delete selected" type="button">
          <img src="<?php echo e(asset('action_icon/delete.svg')); ?>" alt="Delete" width="16" height="16">
        </button>
        <?php endif; ?>
      </div>
    </div>
  ...
  <div class="media-grid" id="mediaGrid">
    <?php $__currentLoopData = $event->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php $url = storage_asset('' . $image->image_path); ?>
      <div class="media-item" data-type="image" data-image-id="<?php echo e($image->id); ?>">
        <input type="checkbox" class="select-check" data-kind="image" value="<?php echo e($image->id); ?>">
        <img class="media-thumb" src="<?php echo e($url); ?>" alt="Image">
        <div class="media-overlay">
          <div class="ovl-actions">
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.view event image')): ?>
              <a class="ovl-btn" href="<?php echo e(route('events.images.open', $image)); ?>" target="_blank" title="Open">
                <img src="<?php echo e(asset('action_icon/show.svg')); ?>" alt="Open">
              </a>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.download event image')): ?>
              <a class="ovl-btn" href="<?php echo e(route('events.images.download', $image)); ?>" title="Download">
            ...
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.delete event image')): ?>
              <button class="ovl-btn" onclick="deleteImage(<?php echo e($image->id); ?>)" title="Delete">
                <img src="<?php echo e(asset('action_icon/delete.svg')); ?>" alt="Delete">
              </button>
            <?php endif; ?>
          </div>
        </div>
    ...
    <?php $__currentLoopData = $event->videos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $video): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php $url = storage_asset('' . $video->video_path); ?>
      <div class="media-item" data-type="video" data-video-id="<?php echo e($video->id); ?>">
        <input type="checkbox" class="select-check" data-kind="video" value="<?php echo e($video->id); ?>">
        <video class="media-thumb" controls preload="metadata" playsinline>
          <source src="<?php echo e(route('events.videos.open', $video)); ?>" type="<?php echo e($video->mime ?? 'video/mp4'); ?>">
        </video>
        <div class="media-overlay">
          <div class="ovl-actions">
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.view event image')): ?>
              <a class="ovl-btn" href="<?php echo e(route('events.videos.open', $video)); ?>" target="_blank" title="Open">
                <img src="<?php echo e(asset('action_icon/view.svg')); ?>" alt="Open">
              </a>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.download event image')): ?>
              <a class="ovl-btn" href="<?php echo e(route('events.videos.download', $video)); ?>" title="Download">
                <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3v12m0 0l4-4m-4 4l-4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 17v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
              </a>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Events Management.delete event image')): ?>
              <button class="ovl-btn" onclick="deleteVideo(<?php echo e($video->id); ?>)" title="Delete">
                <img src="<?php echo e(asset('action_icon/delete.svg')); ?>" alt="Delete">
              </button>
            <?php endif; ?>
          </div>
  ...
  Swal.fire({
    ...
    try {
      ...
      for(const id of sel.images){
        await fetch(`<?php echo e(url('event-images')); ?>/${id}`, { method:'DELETE', headers:{ 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept':'application/json' } });
        const node = grid.querySelector(`[data-image-id="${id}"]`); if(node) node.remove();
      }
      // Delete videos
      for(const id of sel.videos){
        await fetch(`<?php echo e(url('event-videos')); ?>/${id}`, { method:'DELETE', headers:{ 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept':'application/json' } });
...
bulkDownload.addEventListener('click', () => {
  ...
  if(sel.images.length + sel.videos.length === 0) return;
  const urls = [
    ...sel.images.map(id => `<?php echo e(url('event-images')); ?>/${id}/download`),
    ...sel.videos.map(id => `<?php echo e(url('event-videos')); ?>/${id}/download`)
  ];
  let delay = 0;
...
function deleteImage(imageId) {
    Swal.fire({
        ...
        fetch(`<?php echo e(url('event-images')); ?>/${imageId}`, {
            method: 'DELETE',
            headers: {
...
function deleteVideo(videoId) {
    Swal.fire({
        ...
        fetch(`<?php echo e(url('event-videos')); ?>/${videoId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': CSRF_TOKEN,
                'Accept': 'application/json'
...
<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/events/show.blade.php ENDPATH**/ ?>