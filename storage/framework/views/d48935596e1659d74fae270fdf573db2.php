<?php $__env->startSection('footer_pagination'); ?>
  <?php if(isset($inquiries) && method_exists($inquiries,'links')): ?>
  ...
  <?php echo e($inquiries->appends(request()->except('page'))->onEachSide(1)->links('vendor.pagination.jv')); ?>

  <?php endif; ?>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('page_title', 'Inquiry List'); ?>
<?php $__env->startSection('content'); ?>
<div class="inquiry-index-container">
  <!-- JV Filter -->
  <form method="GET" action="<?php echo e(route('inquiries.index')); ?>" class="jv-filter" id="filterForm">
    <input type="text" id="start_date" name="from_date" class="filter-pill date-picker" placeholder="From: dd/mm/yyyy" value="<?php echo e(request('from_date')); ?>" autocomplete="off">
    <input type="text" id="end_date" name="to_date" class="filter-pill date-picker" placeholder="To: dd/mm/yyyy" value="<?php echo e(request('to_date')); ?>" autocomplete="off">
    ...
    <div class="filter-right">
      ...
      <input type="text" class="filter-pill" placeholder="Search here..." id="custom_search" name="search" value="<?php echo e(request('search')); ?>">
      <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.export inquiry')): ?>
        <a href="<?php echo e(route('inquiries.export', request()->only(['from_date','to_date','search']))); ?>" class="pill-btn pill-success" id="excel_btn">Excel</a>
      <?php endif; ?>
      <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.create inquiry')): ?>
        <a href="<?php echo e(route('inquiries.create')); ?>" class="pill-btn pill-success">+ Add</a>
      <?php endif; ?>
    </div>
  </form>

  <?php if(!empty($todayScheduledInquiryIds)): ?>
  ...
  <div class="inquiries-grid-view">
    <?php $__empty_1 = true; $__currentLoopData = $inquiries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <div class="inquiry-grid-card" onclick="window.location.href='<?php echo e(route('inquiries.show', $inq->id)); ?>'" title="View inquiry">
        <div class="inq-grid-header">
          <h3 class="inq-grid-title"><?php echo e($inq->company_name ?? 'N/A'); ?></h3>
          <span class="inq-grid-badge"><?php echo e($inq->industry_type ?? '—'); ?></span>
        </div>
        <p class="inq-grid-sub">Code: <?php echo e($inq->code ?? '-'); ?> • Inq. Date: <?php echo e(isset($inq->inquiry_date) ? \Carbon\Carbon::parse($inq->inquiry_date)->format('d M, Y') : '-'); ?></p>
        <div class="inq-grid-meta">
          <div class="inq-grid-left">
            <div class="meta-row"><span class="meta-label">Contact</span><span class="meta-value"><?php echo e($inq->person_name ?? '-'); ?> <?php echo e(!empty($inq->mobile_no) ? '• '.$inq->mobile_no : ''); ?></span></div>
            <div class="meta-row"><span class="meta-label">Address</span><span class="meta-value"><?php echo e($inq->address ?? '-'); ?></span></div>
            <div class="meta-row"><span class="meta-label">Next FU</span><span class="meta-value"><?php echo e(isset($inq->next_follow_up) ? \Carbon\Carbon::parse($inq->next_follow_up)->format('d M, Y') : '-'); ?></span></div>
          </div>
          <div class="inq-grid-actions" onclick="event.stopPropagation()">
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.view inquiry')): ?>
              <a class="inq-grid-action-btn btn-view" href="<?php echo e(route('inquiries.show', $inq->id)); ?>" title="View" aria-label="View">
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.edit inquiry')): ?>
              <a class="inq-grid-action-btn btn-edit" href="<?php echo e(route('inquiries.edit', $inq->id)); ?>" title="Edit" aria-label="Edit">
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.delete inquiry')): ?>
              <form method="POST" action="<?php echo e(route('inquiries.destroy', $inq->id)); ?>" class="delete-form" style="display:inline">
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Inquiries Management.follow up')): ?>
              <a class="inq-grid-action-btn btn-followup" href="<?php echo e(route('inquiry.follow-up', $inq->id)); ?>" title="Follow Up" aria-label="Follow Up">
            ...
            <?php endif; ?>
            <?php if(!empty($inq->quotation_sent) && strtolower($inq->quotation_sent) !== 'no'): ?>
              <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Quotations Management.create quotation')): ?>
                <a class="inq-grid-action-btn btn-quotation" href="<?php echo e(route('quotation.create-from-inquiry', $inq->id)); ?>" title="Make Quotation" aria-label="Make Quotation">
    ...
    <div class="JV-datatble striped-surface striped-surface--full table-wrap pad-none">
      <table>
        ...
        <tbody id="inquiries-table-body">
          <?php echo $__env->make('inquiries.partials.table_rows', ['inquiries' => $inquiries], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </tbody>
      </table>
    </div>
  </div>
...
<?php echo $__env->make('layouts.macos', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\GitVraj\HrPortal\resources\views/inquiries/index.blade.php ENDPATH**/ ?>