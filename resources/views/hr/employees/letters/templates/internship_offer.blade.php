...
@else
<div class="subject" style="font-size: 13px;">Subject: Internship Offer Letter</div>
@endif

<div class="body">
<p>Dear <b>{{ $employee->name }}</b>,</p>

<p>We are pleased to offer you an internship opportunity with <span class="company">{{ $company_name }}</span>. The details of your internship are as follows:</p>

<p><b>Internship Details:</b></p>
<ul>
    @if($letter->internship_position)
    <li><b>Position:</b> {{ $letter->internship_position }}</li>
    @endif
    @if($letter->internship_start_date)
    <li><b>Start Date:</b> {{ \Carbon\Carbon::parse($letter->internship_start_date)->format('d F Y') }}</li>
    @endif
    @if($letter->internship_end_date)
    <li><b>End Date:</b> {{ \Carbon\Carbon::parse($letter->internship_end_date)->format('d F Y') }}</li>
    @endif
    ...
    @endphp
    <li><b>Duration:</b> {{ $duration }} month{{ $duration != 1 ? 's' : '' }}</li>
    @endif
    @if($letter->working_hours)
    <li><b>Working Hours:</b> {{ $letter->working_hours }}</li>
    @endif
    @if($letter->reporting_manager)
    <li><b>Reporting To:</b> {{ $letter->reporting_manager }}</li>
    @endif
</ul>
...
@if($letter->use_default_content ?? true)
    ...
    <p><b>During your internship, you will:</b></p>
    <ul>
    ...
    <p>We look forward to your contribution and growth with <span class="company">{{ $company_name }}</span>.</p>
@endif
