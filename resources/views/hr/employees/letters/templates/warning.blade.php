...
<head>
    ...
    <title>Warning Letter - {{ $letter->employee->name }}</title>
    <style>
    ...
    <div class="letter-container">
        ...
        <div class="letter-content">
            ...
            <div class="recipient">
                ...
                <div>Designation: {{ $letter->employee->designation ?? 'Employee' }}</div>
            </div>
            <div class="subject">Subject: {{ $letter->title }}</div>
            <div class="body">
                <p>Dear <b>{{ $letter->employee->name }}</b>,</p>
                <p>This letter serves as a <span class="highlight">{{ $letter->content['warning_type'] ?? 'formal warning' }}</span> regarding your conduct/performance at <span class="company">{{ $company_name }}</span>.</p>
                
                <p><b>Details of the Issue:</b></p>
                <ul>
                    <li><b>Date of Incident:</b> {{ isset($letter->content['incident_date']) ? \Carbon\Carbon::parse($letter->content['incident_date'])->format('d-m-Y') : 'As observed' }}</li>
                    <li><b>Nature of Issue:</b> {{ $letter->content['issue_nature'] ?? 'Performance/Conduct related' }}</li>
                    <li><b>Description:</b> {{ $letter->content['description'] ?? 'As discussed' }}</li>
                </ul>

                <p><b>Company Policy Violation:</b></p>
                <p>{{ $letter->content['policy_violation'] ?? 'Your actions/behavior are in violation of company policies and standards.' }}</p>

                <p><b>Expected Improvement:</b></p>
                <ul>
                    <li>{{ $letter->content['improvement_1'] ?? 'Immediate improvement in performance/conduct' }}</li>
                    <li>{{ $letter->content['improvement_2'] ?? 'Adherence to company policies and procedures' }}</li>
                    <li>{{ $letter->content['improvement_3'] ?? 'Professional behavior at all times' }}</li>
                </ul>

                <p><b>Consequences:</b></p>
                <p>Please note that failure to improve may result in further disciplinary action, including but not limited to suspension or termination of employment.</p>

                <p>This warning will remain in your personnel file for {{ $letter->content['warning_validity'] ?? '12 months' }}.</p>

                <p>You have the right to respond to this warning in writing within 7 days of receiving this letter.</p>
            </div>
            <div class="signature">
                ...
                <br>
                <div><b>Employee Acknowledgment:</b></div>
                <div>I acknowledge receipt of this warning letter.</div>
                <br>
                <div>Employee Signature: ___________________ Date: ___________</div>
            </div>
        </div>
    </div>
</body>
