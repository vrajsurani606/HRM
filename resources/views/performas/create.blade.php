  ...
  <div class="Rectangle-30 hrp-compact">
      ...
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem;">
        <div>
          ...
          <input type="text" class="Rectangle-29" name="gst_no" id="gstNoInput" value="{{ old('gst_no', $quotation->gst_no ?? '') }}" placeholder="e.g., 22ABCDE1234F1Z5" style="text-transform: uppercase;" oninput="this.value = this.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 15); toggleGstFields();" maxlength="15" pattern="[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[0-9A-Z]{1}[Z]{1}[0-9A-Z]{1}" title="Enter valid 15-character GST No. (e.g., 22ABCDE1234F1Z5)">
        </div>
        <div>
          <label class="hrp-label">Mobile No :</label>
          <input type="text" class="Rectangle-29 allow-country-code" name="mobile_no" value="{{ old('mobile_no', $quotation->mobile_no ?? '') }}" placeholder="e.g., +919876543210">
        </div>
      </div>
  </div>
